...
findmerced <- ggplot(data= maximum_hpi, aes(x=time, y=change, label= state, colour=state)) + geom_point(size = 2) + geom_text( hjust = 0, vjust = 0, size= 5) + opts(legend.position="none")

pdf("exports/peaktimeVSchange-States.pdf", width = 10, height = 8)
	print(findmerced)
	dev.off()
...
subset(maximum_hpi, state %in% c("CA"))
	...
	mercedgraph <- qplot(x= time, y= hpi, data= merced, geom= "line") + ylim(c(0,350)) + opts(legend.position = "none") 
...
CA <- subset(maximum_hpi, state %in% c("CA"))
	CAgraph <- ggplot(data= CA, aes(x=time, y=change, label= city)) + geom_point(size = 2) + geom_text( hjust = 0, vjust = 0, size= 5) + opts(title = "state == California",legend.position="none") + xlim(c(2005.5,2008))
	 CAgraph
